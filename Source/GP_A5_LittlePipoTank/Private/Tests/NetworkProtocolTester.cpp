...
void ANetworkProtocolTester::TestSerialize()
{
	...
	// Test Serialize FString
	{
		FString Message = "Hello Jérémy and Corentin!";
		UE_LOGFMT(LogTemp, Warning, "Test Serialize FSTRING: {0}", Message);
		UNetworkProtocolHelpers::SerializeString(TestByteArray, Message);
	}
}

void ANetworkProtocolTester::TestDeserialize()
{
	// Test Deserialize Arithmetics
	{
	...
	// Test Deserialize FString
	{
		TArray<BYTE>::SizeType Offset = 0;
		FString Message = UNetworkProtocolHelpers::DeserializeString(TestByteArray, Offset);
		UE_LOGFMT(LogTemp, Warning, "Test Deserialize FSTRING: {0}", Message);
	}
}
